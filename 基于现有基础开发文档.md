...
| 2026-06-02 | develop | correct payment timing after delivery | 纠正订单流程：下单后待发货，发货不依赖付款，签收后才确认支付 | `go test ./...`、`npm run build` 通过 | 随本提交推送到 `origin/develop` |
...
| 2026-06-02 | develop | complete order detail payment view | 第二-三周基础修复：Web 订单详情展示支付状态、实付金额、优惠信息，并支持签收后确认支付 | `npm run build` 通过 | 随本提交推送到 `origin/develop` |
| 2026-06-02 | develop | complete order detail operations | 第二-三周基础修复：Web 订单详情补齐改价、发货、确认签收、签收后确认支付完整操作链路 | `npm run build` 通过 | 随本提交推送到 `origin/develop` |
| 2026-06-02 | develop | align mini program order flow | 第二-三周基础修复：小程序下单页、订单列表、订单详情按“下单待发货、未支付、收货后线下结算”展示，并接入真实确认签收接口 | `node --check` 通过 | 随本提交推送到 `origin/develop` |
