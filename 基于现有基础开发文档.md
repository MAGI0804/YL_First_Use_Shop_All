...
| P0 | 订单支付流程和业务不一致 | 改为“用户下单，运营确认支付” |
...
| 状态 | 中文 | 触发场景 |
| --- | --- | --- |
| `pending` | 待处理 | 用户提交订单后默认状态，等待运营处理 |
...
| 后端状态 | 小程序展示 |
| --- | --- |
| `status=pending` 且 `pay_status=unpaid` | 待处理 |
| `status=pending` 且 `pay_status=paid` | 待发货 |
| `status=shipped` | 待收货 |
...
| 后端状态 | Web 展示 |
| --- | --- |
| `pending + unpaid` | 待处理/未支付 |
| `pending + paid` | 待发货/已支付 |
| `shipped + paid` | 已发货 |
| `delivered + paid` | 已完成 |
| `processing` | 售后中 |
| `canceled` | 已取消 |
...
2. 校验收货人、手机号、地址、商品列表。
3. 调用 `service/method/order_method.go` 的 `CreateOrder`。
4. `CreateOrder` 生成订单号。
5. 序列化商品列表。
6. 计算商品名称列表。
7. 校验库存。
...
当前 `OrderPay` 从 query 取 `order_id`，后续改为 JSON 请求体，同时保留 query 兼容旧调用。
...
2. 校验订单存在。
3. 校验订单未取消。
4. 校验 `pay_status != paid`。
5. 校验 `final_pay_amount >= 0`。
6. 更新主订单：
   - `pay_status = paid`
   - `payment_method = manual`
   - `payment_time = now`
...
7. 同步子订单：`pay_status = paid`。
8. 更新会员累计支付金额，使用 `final_pay_amount`。
9. 返回支付结果。
...
当前后端已有售后单、退货、审核、寄回、收货、库存回滚基础，需要补齐前端操作闭环。
...
| 条件 | 按钮 |
| --- | --- |
| `pending + unpaid` | 调整金额、确认支付、取消订单 |
| `pending + paid` | 发货 |
| `shipped` | 查看物流 |
| `delivered` | 查看详情、查看评价、售后 |
| `processing` | 查看售后 |
...
| `POST /return_order/receive` | 运营确认收货 |
...
当前项目已经具备三端业务基础，不需要重做。后续开发应围绕“上线闭环”推进，核心是修正订单业务模型：用户只下单不支付，运营后台负责最终金额调整、优惠原因记录和手动确认支付。
...
| 日期 | 分支 | 提交 | 范围 | 验证 | 推送状态 |
| --- | --- | --- | --- | --- | --- |
| 2026-06-02 | develop | fbf587e | 第二-三周基础修复第一批：后端订单实付金额、优惠原因、运营改价、确认支付接口、支付校验测试 | `go test ./...` 通过 | `origin/master` 已同步；`develop` upstream 待网络稳定后确认 |
| 2026-06-02 | develop | wire order payment operations UI | 第二-三周基础修复第二批：Web 订单列表展示实付金额/支付状态，接入运营改价和确认支付操作 | `npm run build` 通过 | 待推送 |