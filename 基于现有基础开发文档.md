...
1. 除白名单接口外，小程序请求必须在 URL query 或表单中携带 `access_token`。
2. `/access_token/get_token` 与 `AccessTokenValidationMiddleware` 必须使用同一套客户端 IP 口径：线上代理场景优先取 `X-Forwarded-For` 第一个 IP，取不到时再用 `ClientIP()`。
3. 小程序本地 `access_token` 可能因 Redis 重置、代理 IP 变化、服务端重新发 token 而失效；普通接口遇到 HTTP `401/402` 或响应体 `code=401/402` 时，清除本地 token、重新获取 token，并对原请求最多自动重试一次。
4. 自动重试仍失败时，只回调业务错误，不在通用请求层强制跳首页，避免启动阶段反复路由导致“进小程序什么都无法访问”。
...
7. 首次登录成功后，`member_info.mobile`、`member_info.user_id`、`member_info.openid` 和 `users_user.mobile` 固定绑定；一个手机号只允许对应一个 `user_id`。
   - 后台预先添加会员手机号时，`user_id=0`、`openid=""` 或历史占位值 `openid="0"` 都表示未绑定微信用户，允许首次小程序授权后写入真实 `user_id/openid`。
8. 手机号登录成功后，小程序只在该 `user_id` 第一次成功登录时显示头像昵称补全区；无论用户保存还是跳过，本地都会记录已提示，之后登录直接进入商城。
...
12. 个人信息修改页预加载本地/全局缓存的已上传头像和昵称；用户点击“更换头像”“同步微信头像”“同步微信昵称”只更新页面预览和输入值，只有点击“保存修改”后才请求 `/ordinary_user/Modify_data` 更新后端。

手机号授权排查：

- 小程序端日志若出现 `getPhoneNumber:fail data empty` 且 `hasPhoneCode=false`，表示微信没有返回 `detail.code`，请求不会进入 `/ordinary_user/wechat_login`。
- 此时优先检查运行环境是否为真机预览/体验版、`project.config.json` 的 `appid` 是否为已认证的小程序 AppID、微信公众平台是否已开通手机号快速验证/手机号授权能力。
- 只有 `errMsg=getPhoneNumber:ok` 且 `detail.code` 存在时，后端才能调用微信 `getuserphonenumber` 换取手机号。
...
小程序地址管理页支持从微信收货地址簿同步地址：

1. 页面必须由用户点击“同步微信地址”按钮后调用 `wx.chooseAddress`，不能在页面加载时自动调用。
2. `app.json` 必须声明 `requiredPrivateInfos: ["chooseAddress"]`。
3. 微信返回字段映射：
   ...
   - `cityName` -> `city`
   - `countyName` -> `county`
   - `detailInfo` -> `detailed_address`
4. 小程序拿到微信地址后调用 `POST /address/add_address` 写入当前登录用户地址。
5. 如果当前用户还没有地址，同步进来的第一条地址自动设置为默认地址；已有地址时不覆盖原默认地址。
6. 用户取消选择时不提示错误；授权拒绝时提示用户去设置页开启权限。

相关微信官方文档：

- `wx.chooseAddress`：https://developers.weixin.qq.com/miniprogram/dev/api/open-api/address/wx.chooseAddress.html
- 小程序隐私接口声明：https://developers.weixin.qq.com/miniprogram/dev/reference/configuration/app.html#requiredPrivateInfos
...
- 库存不足提示
- 失效商品提示
- 后台会员详情页从购物车下单时，必须先勾选本次下单商品，不默认全量下单。
...
- 可选择该用户 `addresses` 表中已有地址。
- 支持粘贴整段收货信息，按“收货人、手机号、省市区、详细地址”识别后自动填入表单，识别后仍允许人工修改。
