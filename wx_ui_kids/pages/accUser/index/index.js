        ...
        if (loginRes.code) {
          ...
          req.post('/ordinary_user/wechat_login', {
            ...
            if (res.code === 200) {
              ...
              // 重定向到之前的页面或首页
              console.log('准备跳转，returnPath:', this.returnPath);
              console.log('准备跳转，id:', this.id);
              
              if (this.returnPath === 'goods' && this.id) {
                // 跳转到商品详情页
                const redirectUrl = `/pages/commodity/goods/index?id=${this.id}`;
                console.log('执行跳转:', redirectUrl);
                wx.redirectTo({
                  url: redirectUrl,
