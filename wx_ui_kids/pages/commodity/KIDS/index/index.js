      ...
      app.req.post(
        ...
        (res) => {
          if (res.code === 200) {
            ...
            // 累积商品数据
            const allProducts = pageNum === 1 ? newProducts : [...this.data.products, ...newProducts];
            
            this.setData({
              products: allProducts,
              page: pageNum, // 在响应后才更新页码
              loadingMore: false,
              refresherTriggered: false,
              hasMore: newProducts.length >= pageSize,
              total: totalCount
      ...
      app.req.post(
        ...
        function(res) {
          if (res.code === 200) {
            ...
            that.setData({
              products: allProducts,
              page: requestPage, // 在响应后才更新页码
              loading: false,
              loadingMore: false,
              refresherTriggered: false,
              total: res.data.total || 0,
              hasMore: newProducts.length >= that.data.pageSize // 检查当前页是否有更多数据
            })
            resolve()
          } else {
            that.setData({
