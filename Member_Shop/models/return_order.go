...
type ReturnOrder struct {
	...
	JushuitanAfterSaleID  string     `gorm:"column:jushuitan_after_sale_id;size:50;null;index;comment:聚水潭售后单号" json:"jushuitan_after_sale_id"`
	JushuitanPushStatus   string     `gorm:"column:jushuitan_push_status;size:30;not null;default:'pending';index;comment:聚水潭售后推送状态" json:"jushuitan_push_status"`
	JushuitanPushResponse string     `gorm:"column:jushuitan_push_response;type:text;null;comment:聚水潭售后推送响应" json:"jushuitan_push_response"`
	JushuitanUpdatedTime  *time.Time `gorm:"column:jushuitan_updated_time;null;comment:聚水潭售后最后回写时间" json:"jushuitan_updated_time"`
}
