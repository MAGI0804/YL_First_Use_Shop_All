...
type Cart struct {
	CartID    int          `gorm:"column:cart_id;primaryKey;autoIncrement;comment:购物车ID" json:"cart_id"`
	UserID    int          `gorm:"column:user_id;index;comment:用户ID" json:"user_id"`
	CartItems CartItemsMap `gorm:"column:cart_items;type:json;comment:购物车商品" json:"cart_items"`
	Remarks   string       `gorm:"column:remarks;null;comment:备注" json:"remarks"`
