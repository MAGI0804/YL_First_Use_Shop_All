...
// DjangoOperationUser 运营用户模型
type DjangoOperationUser struct {
	UserID   string `json:"user_id" gorm:"column:user_id;primaryKey;size:6;unique;not null;comment:用户ID"`
	Nickname string `json:"nickname" gorm:"column:nickname;size:100;not null;comment:昵称"`
	Mobile   string `json:"mobile" gorm:"column:mobile;size:20;unique;not null;comment:手机号"`
	Password string `json:"password" gorm:"column:password;size:255;not null;comment:密码"`
	Level    int    `json:"level" gorm:"column:level;not null;comment:级别"`
}
