...
// ReviewMineQueryRequest 我的评价列表查询请求结构体
// 用于小程序用户查询自己的评价记录
type ReviewMineQueryRequest struct {
	UserID   int    `json:"user_id" binding:"required"` // 用户ID，必填
	Status   string `json:"status"`                     // 评价状态
	Page     int    `json:"page"`                       // 页码
	PageSize int    `json:"page_size"`                  // 每页数量
}

// ReviewUpdateRequest 修改待审核评价请求结构体
// 仅允许评价本人修改 pending 状态评价
type ReviewUpdateRequest struct {
	ReviewID uint     `json:"review_id" binding:"required"`          // 评价ID，必填
	UserID   int      `json:"user_id" binding:"required"`            // 用户ID，必填
	Rating   int      `json:"rating" binding:"required,min=1,max=5"` // 评分，1-5分
	Content  string   `json:"content"`                               // 评价内容
	Images   []string `json:"images"`                                // 评价图片列表
	Tags     []string `json:"tags"`                                  // 评价标签列表
}

// ReviewDeleteRequest 删除待审核评价请求结构体
// 仅允许评价本人软删除 pending 状态评价
